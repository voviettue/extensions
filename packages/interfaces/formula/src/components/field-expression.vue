...
<script lang="ts">
import { defineComponent, toRefs, ref, watch, onMounted, onUnmounted, PropType, inject as injectVue } from 'vue';
...
export default defineComponent({
	...
	setup(props, { emit }) {
		...
		const { treeList, loadFieldRelations } = useFieldTree(stores, collection, inject, (field) => {
			return !field?.meta?.special?.includes('m2m') && field?.meta?.interface !== 'input-formula';
		});
