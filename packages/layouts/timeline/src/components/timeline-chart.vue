<template>
	<div id="timeline-layout">
		<div v-if="!!groupField" class="grid-sidebar">
			<div v-for="groupName in Object.keys(groupItems)" :key="`row-${groupName}`" class="group">
				<render-display
		...
		<div ref="grid" class="grid">
			...
			<div class="grid-body" :style="`width: ${colWidth * dates.length}px`">
				<div v-for="groupName in Object.keys(groupItems)" :key="`row-${groupName}`" class="row">
					<div class="row-grid" :style="`grid-template-columns: repeat(${dates.length}, ${colWidth}px)`">
						<div
							...
							"
						>
							<v-avatar v-if="userField && colWidth > 28" class="avatar" style="--v-avatar-size: 20px">
								<img v-if="avatarSrc(item[userField])" :src="avatarSrc(item[userField])" />
				...
				<div v-show="isOnHover" ref="tooltip" class="item-toolip">
					<template v-if="!!hoveringItem">
						...
						<div>
							<v-chip v-for="tag in hoveringItem[tagField] || []" :key="tag" x-small class="mr-5 mb-5">
								{{ tag }}
							</v-chip>
						</div>
						<div class="text-gray mb-5 t12">
							{{
								hoveringItem[startDateField] ? formatDate(new Date(hoveringItem[startDateField]), 'dd MMM yyyy') : '/'
							}}
							-
							{{
								hoveringItem[endDateField] ? formatDate(new Date(hoveringItem[endDateField]), 'dd MMM yyyy') : 'Endless'
							}}
						</div>
						<div class="d-flex">
							<v-avatar v-if="userField && viewMode !== 'year'" class="avatar mr-5" x-small :size="20">
								<img v-if="avatarSrc(hoveringItem[userField])" :src="avatarSrc(hoveringItem[userField])" />
								<v-icon v-else name="person_outline" />
							</v-avatar>
							<span v-if="!!hoveringItem[userField]" class="t12">{{ `${hoveringItem[userField].email}` }}</span>
						</div>
					</template>
				</div>
			</div>
...
import { format, isLastDayOfMonth, lastDayOfYear, differenceInCalendarDays } from 'date-fns';
...
export default defineComponent({
	...
	computed: {
		...
		headers() {
			...
			switch (this.viewMode) {
					...
					this.dates.forEach((date, index) => {
						if (date.getDay() === 0 || this.dates.length === index + 1) {
							headers.push({
								...
								label: `${this.dates[start].getDate()} - ${this.dates[index].getDate()}`,
				...
				case 'year':
					this.dates.forEach((date, index) => {
						if (lastDayOfYear(date).toDateString() === date.toDateString() || this.dates.length === index + 1) {
							headers.push({
								ranges: this.dates.slice(start, index + 1),
	...
	watch: {
		...
		startDate: function (value, newValue) {
			const days = differenceInCalendarDays(new Date(newValue), new Date(value));
			this.$nextTick(() => {
				if (days > 0) {
	...
	methods: {
		...
		scrollToToday() {
			...
			if (todayColumn) {
				const grid = this.$refs.grid;
				this.scrollTo(todayColumn.offsetLeft - Math.round(grid.offsetWidth / 2));
			}
		},
		initRowHeight() {
		...
		avatarSrc(user: any): string | null {
			...
			if (user?.avatar?.id) {
				return this.addTokenToURL(`${getRootPath()}assets/${user.avatar.id}?key=system-medium-cover`);
			}
		...
		getToken(): string | null {
			return this.api.defaults.headers.common['Authorization']?.split(' ')[1] || null;
		},
		...
		matchedConditions(item) {
			return (this.conditionalStyles || []).find(({ field, operator, value }) => {
