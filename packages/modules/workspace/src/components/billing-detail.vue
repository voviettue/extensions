  ...
  <div class="box">
    <div class="inline">
      <div class="title">
        ...
        <v-chip
          :class="{ chip: true, success: subscription.status === 'active' }"
          :small="true"
          >{{ subscription.status.toUpperCase() }}</v-chip
        >
      </div>
      <v-button
        class="btn-portal"
        ...
        :loading="isloading"
        v-tooltip="
          'View invoices, update payment methods, and billing information'
        "
        @click="goToPortal"
      >
    ...
    <div class="inline">
      ...
      <div v-if="upcomingInvoice" class="border-left pl-20 ml-20">
        ...
        <strong>
          {{
            `${symbols[subscription.plan.currency]}${
