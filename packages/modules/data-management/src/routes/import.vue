  ...
  <private-view title="Import data">
    ...
    <div class="padding-box">
      ...
      >
        ...
        <template #append>
          <v-button to="/settings/data-model/+">{{
            t("create_collection")
          }}</v-button>
        </template>
      </v-info>
      ...
      <v-list v-else>
        ...
        >
          <v-list-item-icon>
            <v-icon
              :name="collection.meta.icon || 'label'"
...
export default {
  ...
  setup() {
    ...
    const collections = collectionStore.visibleCollections.filter(
      (e) => e.type !== "alias"
    );
    const { t } = useI18n();
    ...
    function onClickCollection(collection) {
      router.push(`/import/${collection.collection}`);
    }
    ...
    function createAllowed(collection) {
      ...
      const createPermissions = permissionsStore.permissions.find(
        (permission) =>
          ...
          permission.collection === collection.collection
      );
